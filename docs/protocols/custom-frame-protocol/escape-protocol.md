...
3. [核心机制详解](#3-核心机制详解)
   - [3.1 整体流程](#31-整体流程)
   - [3.2 转义机制](#32-转义机制)
   - [3.3 粘包解决机制](#33-粘包解决机制)
   - [3.4 丢包解决机制](#34-丢包解决机制)
4. [示例代码](#4-示例代码)
5. [协议优势](#5-协议优势)
6. [总结](#6-总结)
...
stateDiagram-v2
    [*] --> IDLE: 初始化
    IDLE --> HEAD_MATCHED: 收到 0xAA
    IDLE --> IDLE: 收到其他字节
    HEAD_MATCHED --> RECV_DATA: 收到长度字节
    RECV_DATA --> CHECK_TAIL: 收到指定数量字节
    CHECK_TAIL --> VALIDATE: 收到 0x55
    CHECK_TAIL --> RECV_DATA: 未收到帧尾
    VALIDATE --> IDLE: 校验成功
    VALIDATE --> IDLE: 校验失败,请求重传
```
...
- [工程师进阶笔记 - MCU 字节流通信协议设计](https://mp.weixin.qq.com/s/fE0SPLjWFQJ_K_e8wvuTkQ)
